...
export const getAllTasks = (token, id) => async (dispatch) => {
  ...
  try {
    const response = await axios.get("/api/task/tasks", config);
...
export const editTask = (taskId, updatedTask) => async (dispatch) => {
  try {
    const response = await axios.put(`/api/task/edit/${taskId}`, {
      task: updatedTask,
    });
...
export const arrowClick = (item, string) => async () => {
  ...
  try {
    let response = await axios.put(`/api/task/${taskData.id}`, taskData);
